    ...
    if (event_base == WIFI_EVENT) {
        switch (event_id) {
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
                if (retry_num < STA_MAX_RETRY) {
                    ...
                    ESP_LOGI(
                        WIFI_TAG,
                        "Failed to connect err: %s. Retrying to connect to the "
                        "AP",
                        esp_err_to_name(err)
                    );
                } else {
        ...
        switch (event_id) {
            case IP_EVENT_STA_GOT_IP: {
                ...
                ESP_LOGI(
                    WIFI_TAG, "STA Got ip:" IPSTR, IP2STR(&event->ip_info.ip)
                );
                retry_num = 0;
...
void wifiInitSTA(const char* sta_ssid, const char* sta_password) {
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
        WIFI_EVENT, ESP_EVENT_ANY_ID, &eventHandlerSTA, NULL, &instance_any_id
    ));
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
        IP_EVENT, IP_EVENT_STA_GOT_IP, &eventHandlerSTA, NULL, &instance_got_ip
    ));
...
void wifiInitAP(const char* ap_ssid, const char* ap_password) {
    ...
    esp_event_handler_instance_t instance_any_id;
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
        WIFI_EVENT, ESP_EVENT_ANY_ID, &eventHandlerAP, NULL, &instance_any_id
    ));
...
void wifiInitSTAAP(
    const char* sta_ssid, const char* sta_password, const char* ap_ssid,
    ...
    esp_event_handler_instance_t instance_got_ip;
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
        WIFI_EVENT, ESP_EVENT_ANY_ID, &eventHandlerSTAAP, NULL, &instance_any_id
    ));
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
        IP_EVENT, IP_EVENT_STA_GOT_IP, &eventHandlerSTAAP, NULL,
        &instance_got_ip
    ));
